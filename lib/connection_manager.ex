defmodule RtspToWebRTC.ConnectionManager do
  use Connection
  require Logger
  alias Membrane.RTSP
  ...
  def start_link(args) do
    Logger.info("ConnectionManager: start_link, args: #{inspect(args)}")
  ...
  def disconnect(message, %ConnectionStatus{} = connection_status) do
    Logger.info("ConnectionManager: Disconnecting: #{message}")
  ...
  defp get_rtsp_description(%ConnectionStatus{rtsp_session: rtsp_session} = connection_status) do
    Logger.info("ConnectionManager: Setting up RTSP description")
    ...
    case RTSP.describe(rtsp_session) do
      ...
      result ->
        Logger.error("Failed to get description: #{inspect(result, pretty: true)}")
  ...
  defp setup_rtsp_connection(%ConnectionStatus{
    ...
    Logger.info("ConnectionManager: Setting up RTSP connection")
    ...
    case RTSP.setup(rtsp_session, "/#{pipeline_options[:control]}", [
      ...
      result ->
        Logger.info("ConnectionManager: Setting up RTSP connection failed: #{inspect(result)}")
  ...
  defp play(%ConnectionStatus{rtsp_session: rtsp_session, pipeline: _pipeline}) do
    Logger.info("ConnectionManager: Setting RTSP on play mode")
  ...
  defp start_keep_alive(%ConnectionStatus{rtsp_session: rtsp_session} = connection_status) do
    Logger.info("ConnectionManager: Starting Keep alive process")
